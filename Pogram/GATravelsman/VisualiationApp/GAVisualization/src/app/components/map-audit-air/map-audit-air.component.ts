...
@Component({
  selector: 'map-audit-air',
  templateUrl: './map-audit-air.component.html',
  styleUrls: ['./map-audit-air.component.scss']
})

export class MapAuditAirComponent implements OnInit {
  ...
  runGeneticAlgorithmPath() {
    ...
    crossedPopulations.forEach(x => {
        ...
        origins.forEach(y => {
          let point = this.defaultRoutePoints.find(x => x.id == y);
          if (point) {
